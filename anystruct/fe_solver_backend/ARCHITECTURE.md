...
```python
from fe_solver import DNVC208MaterialCurve, solve_static_nonlinear

model.add_material("S355", 210e9, 0.3,
                   hardening_curve=DNVC208MaterialCurve(
                       320e6, 357e6, 363.3e6, 0.004, 0.015, 740e6, 0.166))
result = solve_static_nonlinear(model, live_load,
                                constant_load_case=dead_load,
                                max_load_factor=1.0, num_steps=10,
                                num_layers=5)
```
...
- **Material nonlinearity**: layered J2 plane-stress plasticity in the
  ...
  once.  Materials without a curve stay elastic; beams are elastic.
- **Solution control**: full Newton-Raphson per increment with a
  backtracking-line-search rescue retry, adaptive load-step cutback/growth,
  ``F = F_constant + lambda * F_proportional`` load split, and plastic state
  committed only on increment convergence.  The reported
  ``capacity_estimate`` is the last converged load factor.
