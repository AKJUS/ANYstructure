...
def test_report_data_snapshot_keeps_plain_report_input_data():
    snapshot = project_services.ReportDataSnapshot(
        ...
        buckling_method="ML-CL (SemiAnalytical based)",
        points={"point1": [0, 0]},
        lines={"line1": [1, 2]},
        line_bundles={"line1": ["bundle"]},
        tanks={"comp1": "tank"},
    ...
    assert snapshot.buckling_method == "ML-CL (SemiAnalytical based)"
...
def test_project_open_transfer_service_normalizes_saved_view_payloads():
    ...
    assert transfer.buckling_method == "ML-CL (SemiAnalytical based)"
